...
In a web build, `HtmlUI.jslib` asks Chrome to composite a DOM subtree straight into a WebGL or WebGPU texture
...
HtmlDocument / HtmlElement / HtmlScreenSurface / HtmlWorldSurface   (unchanged)
                      ...
                      HtmlNative
                     /            \
   [DllImport] HtmlUI.jslib     HtmlBackend.Current  ->  CdpHtmlBackend  ->  Chrome
     (web builds)                  (Editor)
```
...
| `Editor/Cdp/HtmlUI-EditorPremultiply.shader` | The blit that turns a screencast frame into a runtime texture. |
...
`HtmlRuntime.Initialize` then calls `HtmlUI_Init`, which reaches `CdpHtmlBackend.Init`. That returns
...
Set `HTMLUI_CHROME` (or `CHROME_PATH`) to override executable discovery.
...
1. `Target.createTarget {url: "about:blank"}` — deliberately without `width`/`height`, which Chrome rejects
   unless it is opening a new window. The viewport is set below instead.
...
Three things happen in `Hidden/HtmlUI/EditorPremultiply`, and each has a reason:

**Premultiply.** The runtime's shaders (`HtmlUI/UI Premultiplied`, `HtmlUI/Unlit Premultiplied`) blend
...
3. `HtmlRuntime.LateUpdate` → `HtmlUI_Update()` → `CdpHtmlBackend.Update()`:
...
`HTMLUI_CHROME`. `HtmlDocument`'s inspector shows the preview status while playing.

**The UI is upside down.** Toggle **Window > HTML UI > Flip Preview Vertically**.
