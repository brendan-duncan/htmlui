# HTML UI for Unity Web (HTML-in-Canvas)

Define Unity UIs in **HTML and CSS** for WebGL2 and WebGPU builds. The package hosts real DOM inside the Unity
...
1. Add the package (it is an embedded package in `Packages/com.brendan.htmlui`).
...
| `HtmlScreenSurface` | Draws the document through a uGUI `RawImage` (requires the `HtmlUI/UI Premultiplied` shader, assigned automatically) and syncs geometry to the RectTransform. |
| `HtmlWorldSurface` | Draws the document on a mesh with `HtmlUI/Unlit Premultiplied` and syncs perspective geometry. |
