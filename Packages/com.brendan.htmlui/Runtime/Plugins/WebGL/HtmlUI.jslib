// HTML UI for Unity Web (WebGL / WebGPU) - browser side bridge.
//
...
var HtmlUILibrary = {
  ...
  $HUI: {
    ...
    // JS string -> freshly malloc'd UTF8 buffer. The C# side copies it and calls HtmlUI_Free.
    cstr: function (s) {
    ...
    injectBaseStyle: function () {
      ...
      s.textContent =
        ...
        '.hui-overlay{position:fixed;left:0;top:0;width:0;height:0;overflow:hidden;pointer-events:none;z-index:1000;}' +
    ...
    updateOverlay: function () {
      ...
      if (o[0] === r.left && o[1] === r.top && o[2] === r.width && o[3] === r.height) return;
      HUI.overlayRect = [r.left, r.top, r.width, r.height];
      HUI.overlay.style.left = r.left + 'px';
      HUI.overlay.style.top = r.top + 'px';
  ...
  HtmlUI_Init: function (backend, linear, forceOverlay, debug, eventCb) {
    HUI.eventCb = eventCb;
    return HUI.init(backend, !!linear, !!forceOverlay, !!debug);
  },

  HtmlUI_GetFeatures: function () {
    return HUI.cstr(JSON.stringify(HUI.features || {}));
  },

  HtmlUI_GetCanvasInfo: function (outPtr) {
    var c = HUI.canvas;
    var o = outPtr >> 2;
  ...
  HtmlUI_SetUpdateMode: function (mode) { HUI.updateMode = mode | 0; },
  HtmlUI_SetGeometryMode: function (mode) {
    HUI.geometryMode = mode | 0;
    for (var id in HUI.panels) HUI.panels[id].lastMatrix = null; // re-apply with the new strategy
  },

  HtmlUI_Update: function () { HUI.update(); },

  HtmlUI_Free: function (ptr) { if (ptr) _free(ptr); },

  // ---- panels

  HtmlUI_PanelCreate: function (w, h) { return HUI.createPanel(w, h); },
  HtmlUI_PanelDestroy: function (id) { HUI.destroyPanel(id); },
  HtmlUI_PanelSetHtml: function (id, htmlPtr) { var p = HUI.panel(id); if (p) HUI.setHtml(p, UTF8ToString(htmlPtr)); },
  HtmlUI_PanelSetCss: function (id, cssPtr) { var p = HUI.panel(id); if (p) HUI.setCss(p, UTF8ToString(cssPtr)); },
  HtmlUI_PanelSetSize: function (id, w, h) { var p = HUI.panel(id); if (p) HUI.setSize(p, w, h); },
  HtmlUI_PanelSetVisible: function (id, v) { var p = HUI.panel(id); if (p) HUI.setVisible(p, !!v); },
  HtmlUI_PanelSetPointerMode: function (id, mode) {
    var p = HUI.panel(id); if (!p) return;
    p.el.setAttribute('data-hui-pointer', mode === 0 ? 'panel' : (mode === 1 ? 'children' : 'none'));
  },
  HtmlUI_PanelSetBlockInput: function (id, block) { var p = HUI.panel(id); if (p) HUI.setBlockInput(p, !!block); },
  HtmlUI_PanelSetPremultiplied: function (id, v) { var p = HUI.panel(id); if (p) { p.premultiply = !!v; p.dirty = true; } },
  // Texture density relative to the device pixel ratio (2 = supersample). Takes effect on the next SetSize.
  HtmlUI_PanelSetResolutionScale: function (id, scale) { var p = HUI.panel(id); if (p) p.scale = Math.max(0.25, Math.min(4, scale || 1)); },
  HtmlUI_PanelSetMipmaps: function (id, v) { var p = HUI.panel(id); if (p) { p.mipmaps = !!v; p.dirty = true; } },
  // Returns 1 once after each successful texture upload (WebGPU mip generation is done from C#).
  HtmlUI_PanelTakeUpdated: function (id) { var p = HUI.panel(id); if (!p || !p.updated) return 0; p.updated = false; return 1; },
  HtmlUI_PanelSetPreventSubmit: function (id, v) { var p = HUI.panel(id); if (p) p.preventSubmit = !!v; },
  HtmlUI_PanelListen: function (id, typePtr, enabled) { var p = HUI.panel(id); if (p) HUI.listen(p, UTF8ToString(typePtr), !!enabled); },
  HtmlUI_PanelInvalidate: function (id) { var p = HUI.panel(id); if (p) { p.dirty = true; HUI.requestPaint(); } },
  HtmlUI_PanelSetGeometry: function (id, matPtr) {
    var p = HUI.panel(id); if (!p) return;
    HUI.setGeometry(p, HEAPF32.subarray(matPtr >> 2, (matPtr >> 2) + 16));
  },
  HtmlUI_PanelGetTextureSize: function (id, outPtr) {
    var p = HUI.panel(id);
  ...
  HtmlUI_PanelCreateGLTexture: function (id) { var p = HUI.panel(id); return p ? HUI.allocGLTexture(p) : 0; },
  // WebGPU: Unity owns the texture, the bridge draws into it.
  HtmlUI_PanelBindGPUTexture: function (id, ptr) { var p = HUI.panel(id); if (p) { p.gpuPtr = ptr; p.dirty = true; HUI.requestPaint(); } },
  HtmlUI_PanelAnnounce: function (id, textPtr, assertive) { var p = HUI.panel(id); if (p) HUI.announce(p, UTF8ToString(textPtr), !!assertive); },
  HtmlUI_PanelEval: function (id, codePtr) {
  ...
  HtmlUI_Query: function (id, selPtr) {
  ...
  HtmlUI_QueryAll: function (id, selPtr) {
    var p = HUI.panel(id); if (!p) return HUI.cstr('');
    var out = [];
    try {
  ...
  HtmlUI_ElemRelease: function (h) { HUI.releaseHandle(h); },
  HtmlUI_ElemEnsureId: function (h) { var e = HUI.el(h); return HUI.cstr(e ? HUI.ensureId(e) : ''); },
  HtmlUI_ElemGetText: function (h) { var e = HUI.el(h); return HUI.cstr(e ? e.textContent : ''); },
  HtmlUI_ElemSetText: function (h, sPtr) { var e = HUI.el(h); if (e) e.textContent = UTF8ToString(sPtr); },
  HtmlUI_ElemGetHtml: function (h) { var e = HUI.el(h); return HUI.cstr(e ? e.innerHTML : ''); },
  HtmlUI_ElemSetHtml: function (h, sPtr) { var e = HUI.el(h); if (e) e.innerHTML = UTF8ToString(sPtr); },
  HtmlUI_ElemInsertHtml: function (h, wherePtr, sPtr) {
    var e = HUI.el(h); if (e) e.insertAdjacentHTML(UTF8ToString(wherePtr), UTF8ToString(sPtr));
  },
  HtmlUI_ElemGetAttr: function (h, nPtr) { var e = HUI.el(h); var v = e ? e.getAttribute(UTF8ToString(nPtr)) : null; return HUI.cstr(v === null ? '' : v); },
  HtmlUI_ElemHasAttr: function (h, nPtr) { var e = HUI.el(h); return (e && e.hasAttribute(UTF8ToString(nPtr))) ? 1 : 0; },
  HtmlUI_ElemSetAttr: function (h, nPtr, vPtr) { var e = HUI.el(h); if (e) e.setAttribute(UTF8ToString(nPtr), UTF8ToString(vPtr)); },
  HtmlUI_ElemRemoveAttr: function (h, nPtr) { var e = HUI.el(h); if (e) e.removeAttribute(UTF8ToString(nPtr)); },
  HtmlUI_ElemGetProp: function (h, nPtr) { var e = HUI.el(h); var v = e ? e[UTF8ToString(nPtr)] : undefined; return HUI.cstr(v === undefined || v === null ? '' : String(v)); },
  HtmlUI_ElemSetProp: function (h, nPtr, vPtr) { var e = HUI.el(h); if (e) e[UTF8ToString(nPtr)] = UTF8ToString(vPtr); },
  HtmlUI_ElemSetBoolProp: function (h, nPtr, v) { var e = HUI.el(h); if (e) e[UTF8ToString(nPtr)] = !!v; },
  HtmlUI_ElemGetBoolProp: function (h, nPtr) { var e = HUI.el(h); return (e && !!e[UTF8ToString(nPtr)]) ? 1 : 0; },
  HtmlUI_ElemSetStyle: function (h, nPtr, vPtr) { var e = HUI.el(h); if (e) e.style.setProperty(UTF8ToString(nPtr), UTF8ToString(vPtr)); },
  HtmlUI_ElemGetStyle: function (h, nPtr) { var e = HUI.el(h); return HUI.cstr(e ? getComputedStyle(e).getPropertyValue(UTF8ToString(nPtr)) : ''); },
  HtmlUI_ElemAddClass: function (h, cPtr) { var e = HUI.el(h); if (e) e.classList.add(UTF8ToString(cPtr)); },
  HtmlUI_ElemRemoveClass: function (h, cPtr) { var e = HUI.el(h); if (e) e.classList.remove(UTF8ToString(cPtr)); },
  HtmlUI_ElemToggleClass: function (h, cPtr, force) { var e = HUI.el(h); if (e) e.classList.toggle(UTF8ToString(cPtr), force < 0 ? undefined : !!force); },
  HtmlUI_ElemHasClass: function (h, cPtr) { var e = HUI.el(h); return (e && e.classList.contains(UTF8ToString(cPtr))) ? 1 : 0; },
  HtmlUI_ElemFocus: function (h) { var e = HUI.el(h); if (e && e.focus) e.focus({ preventScroll: true }); },
  HtmlUI_ElemBlur: function (h) { var e = HUI.el(h); if (e && e.blur) e.blur(); },
  HtmlUI_ElemClick: function (h) { var e = HUI.el(h); if (e && e.click) e.click(); },
  HtmlUI_ElemRemove: function (h) { var e = HUI.el(h); if (e && e.parentNode) e.parentNode.removeChild(e); HUI.releaseHandle(h); },
  HtmlUI_ElemShowModal: function (h, v) {
  ...
  HtmlUI_ElemGetBounds: function (h, outPtr) {
    var e = HUI.el(h);
    var o = outPtr >> 2;
  ...
  HtmlUI_ElemQuery: function (h, selPtr) {
    var e = HUI.el(h); if (!e) return 0;
    var r = null;
    try { r = e.querySelector(UTF8ToString(selPtr)); } catch (ex) {}
    return HUI.handle(r);
  },
  HtmlUI_ElemParent: function (h) { var e = HUI.el(h); return HUI.handle(e && e.parentElement && !e.parentElement.classList.contains('hui-content') ? e.parentElement : null); },
  HtmlUI_ElemMatches: function (h, selPtr) { var e = HUI.el(h); try { return (e && e.matches(UTF8ToString(selPtr))) ? 1 : 0; } catch (ex) { return 0; } },
  HtmlUI_ElemScrollIntoView: function (h) { var e = HUI.el(h); if (e && e.scrollIntoView) e.scrollIntoView({ block: 'nearest' }); }
};

autoAddDeps(HtmlUILibrary, '$HUI');
mergeInto(LibraryManager.library, HtmlUILibrary);
