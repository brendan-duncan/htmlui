...
`HtmlUI.jslib` and the C# that drives it. For the Editor's Chrome-over-DevTools stand-in, see
...
placed in a `position: fixed` layer tracking the canvas rect. The UI is fully accessible and interactive; it is
simply drawn *over* the Unity frame instead of composited into it, so it cannot sit on a 3D quad or be occluded
by scene geometry. `HtmlDocument.Texture` is null and surfaces disable their renderers.
...
HtmlRuntime                                     one tick per frame, event routing
   ...
   HtmlUI.jslib  ($HUI)                         DOM, textures, geometry, input
        |
   Chrome
```

The ABI is deliberately narrow: ints, floats, C strings and one function pointer. Strings returned from JS are
`_malloc`'d by `HUI.cstr` and freed by the caller through `HtmlUI_Free`, which is what `HtmlNative.TakeString`
...
`HtmlDocument.Create` calls `HtmlUI_PanelCreate`, which builds this, once per document:
...
DOM changes  ->  browser repaints  ->  canvas 'paint' {changedElements}  ->  panel.dirty = true
                                                                                  |
Unity LateUpdate -> HtmlUI_Update() -> for each dirty visible panel: upload -> panel.dirty = false
```
...
Reversed: C# creates a `RenderTexture` and passes `GetNativeTexturePtr()` to `HtmlUI_PanelBindGPUTexture`. The
...
`HUI.handles` is an array of element references with a free list; `HtmlUI_Query` returns an index. The element
...
`HtmlUI/UI Premultiplied` and `HtmlUI/Unlit Premultiplied` and the surfaces assign them automatically. Turning
...
3. `HtmlRuntime.LateUpdate` — refresh canvas size and DPR, then `HtmlUI_Update()`: upload every dirty visible
   panel.
4. `HtmlDocument.AfterBridgeUpdate` — regenerate mips for WebGPU render textures that were actually updated
   (`HtmlUI_PanelTakeUpdated`).
